        ...
        <article className='dark:border-light  dark:bg-dark rounded-br-3xl relative w-full p-12 flex items-center justify-between rounded-3xl border border-solid border-dark bg-light shadow-2xl'>
            ...
            <div className='w-1/2 rounded-br-3xl flex flex-col items-start justify-between pl-6'>
                ...
                <Link href={link} target="_blank" className='hover:underline under underline-offset-2'>
                    <h2 className='my-2 w-full dark:text-light text-left text-4xl font-bold'>{title}</h2>
                </Link>
                <p className='my-2 dark:text-light font-medium text-dark'>{summary}</p>

                <div className='mt-2 flex items-center'>
                <Link className="w-10 dark:bg-light dark:rounded-full" href={github} target="_blank"><GithubIcon/></Link>
                <Link className= 'ml-4 rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold dark:text-dark dark:bg-white dark:border-light' href={link} target="_blank">Visit Project</Link>
                </div>
            </div>
        </article>
    )
...
const Project = ({type,title,img,link,github}) => {
    return(
        <article className='dark:border-light dark:bg-dark rounded-br-3xl w-full flex-col p-6 relative flex items-center justify-center rounded-2xl border border-solid border-dark bg-light shadow-2xl'>
            <div className='absolute dark:bg-light top-0 -right-3 -z-10 w-[101%] h-[103%] rounded-[2rem] bg-dark'/>
            ...
            <div className='w-full flex flex-col items-start justify-between mt-4'>
                <span className='text-primary dark:text-primaryDark font-medium text-xl'>{type}</span>
                <Link href={link} target="_blank" className='hover:underline under underline-offset-2'>
                    <h2 className='my-2 w-full dark:text-white text-left text-3xl font-bold'>{title}</h2>
                </Link>

                <div className='mt-2 flex items-center justify-between w-full'>
                    <Link className='dark:text-dark dark:bg-white dark:border-light rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold' href={link} target="_blank">Visit</Link>
                    <Link className="w-8 dark:bg-light dark:rounded-full" href={github} target="_blank"><GithubIcon/></Link>
                </div>
            </div>
        </article>
    )
